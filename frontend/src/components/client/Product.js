...
function Product({setClickedProduct, setClicked, product}) {
  return (
    <div className="border-l-2 border-red-400 hover:border-red-600 duration-300 hover:shadow-xl hover:-translate-y-2 transform shadow-lg w-80 bg-white p-4 flex flex-col gap-2">
      ...
      {/* <Link to={`/product/${product._id}`}> */}
        <button onClick={() => {setClicked(true); setClickedProduct(product["_id"])}} className="hover:bg-white shadow-lg hover:text-black duration-300 bg-black w-full mx-auto py-2 text-white">Order Now ${product.price}</button>
      {/* </Link> */}
    </div>
  )
}
